CREATE OR REPLACE FUNCTION marlin.get_enriched_measurements(
    ...
    STABLE
    LANGUAGE plpgsql
    -- FIX: Explicitly set the path so it finds 'geometry' and 'ST_Y' in 'marlin'
    SET search_path = marlin, public
AS
$$
BEGIN
    RETURN QUERY
        WITH raw_data AS (
            SELECT * FROM marlin.get_measurements(p_time_range, p_location_id, p_sensor_id, p_type_id)
        )
        SELECT
            rd.location_id,
